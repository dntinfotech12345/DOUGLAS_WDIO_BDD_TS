...
export class Assert {
    ...
    async assertTitleContains(titlePart: string) {
        const actualTitle = await browser.getTitle();
        expect(actualTitle).to.include(titlePart, `Expected title to contain '${titlePart}', but found '${actualTitle}'`);
    }
    ...
    async assertURLContains(urlPart: string) {
        const currentURL = await browser.getUrl();
        expect(currentURL).to.include(urlPart, `Expected URL to contain '${urlPart}', but found '${currentURL}'`);
    }
    ...
    async assertElementContainsText(locator: string, expectedText: string) {
        ...
        expect(actualText).to.include(expectedText, `Expected element text to contain '${expectedText}', but found '${actualText}'`);
    }
